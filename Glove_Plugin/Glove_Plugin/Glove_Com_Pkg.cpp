...
#define GLOVEDLL_API __declspec(dllexport)
//define _WIN32
...
extern "C" {
	...
	*/
	...
	GLOVEDLL_API void o(int n);
	GLOVEDLL_API void c();
	void g(int i);
	GLOVEDLL_API void b();
	GLOVEDLL_API void e();
	GLOVEDLL_API char* rd();
	GLOVEDLL_API int dataIn();
	char* str(char ch, int rem);
	...
	GLOVEDLL_API void o(int n) { //(o)pen:open serial port comn
	...
	GLOVEDLL_API void c() { //(c)lose:close the currently open serial port comn
	...
	int iii;
	GLOVEDLL_API void b() { //(b)egin:request to begin sending data
		RS232_cputs(cport_nr, "\xa5\x5a\x10");
		iii = 0;
	}

	GLOVEDLL_API void e() { //(e)nd:request to end sending data
	...
	GLOVEDLL_API  char* unityGlove() {
		return(h);
	}

	GLOVEDLL_API char* rd() { //rd: read
		...
		if (n > 0) {
			...
			for (si = 0; si < n; si++) {
				rem = n - si;
				char* temp = str(buf[si], rem);
				if (temp != NULL) { //packet ready
